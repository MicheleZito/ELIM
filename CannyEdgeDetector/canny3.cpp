...
///////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				// 	if(67.5 < direction.at<float>(i,j) && direction.at<float>(i,j) <= 112.5) //orizzontale
				// 	{
				// 	// controllo che i pixel che seguono il bordo su cui dice di trovarsi il pixel preso i considerazione
				// 	// non siano già stati controllati in precedenza e che siano interni per valore di magnitudo al range stabilito da Tlow e Thigh
				// 	// e che abbiano una direzione pari a quella del pixel considerato
				// 		k = i;
				// 		m = j-1;
				// 		if( /*nonmaxsub.at<float>(k,m) != 100.0 &&*/ nonmaxsub.at<float>(k,m) == 80.0 && 
				// 			// Tlow < magnitudo.at<float>(k,m) && magnitudo.at<float>(k,m) < Thigh && 
				// 			67.5 < direction.at<float>(k,m) && direction.at<float>(k,m) <= 112.5)// && 
				// 			// magnitudo.at<float>(k,m) > magnitudo.at<float>(k-1,m) &&
				// 			// magnitudo.at<float>(k,m) > magnitudo.at<float>(k+1,m)) 
				// 		{
				// 			value = true;					
				// 			nonmaxsub.at<float>(k,m) = 255.0;
				// 		}
				...
				// 	else if(( 0.0 < direction.at<float>(i,j) && direction.at<float>(i,j) <= 22.5 ) ||
				// 		(157.5 < direction.at<float>(i,j) && direction.at<float>(i,j) <= 180.0)) // verticale
				// 	{
				...
				// 		if( /*nonmaxsub.at<float>(k,m) != 100.0 &&*/ nonmaxsub.at<float>(k,m) == 80.0  &&
				// 			// Tlow < magnitudo.at<float>(k,m) && magnitudo.at<float>(k,m) < Thigh &&
				// 			((0.0 < direction.at<float>(k,m) && direction.at<float>(k,m) <= 22.5) ||
				// 			(157.5 < direction.at<float>(k,m) && direction.at<float>(k,m) <= 180.0)) )//&&
				// 			// magnitudo.at<float>(k,m) > magnitudo.at<float>(k,m-1) &&
				// 			// magnitudo.at<float>(k,m) > magnitudo.at<float>(k,m+1)) 
				// 		{												
				// 			value = true;							 
				// 			nonmaxsub.at<float>(k,m) = 255.0;
				...
				// 		if( /*nonmaxsub.at<float>(k,m) != 100.0 &&*/ nonmaxsub.at<float>(k,m) == 80.0 && 
				// 			// Tlow < magnitudo.at<float>(k,m) && magnitudo.at<float>(k,m) < Thigh && 
				// 			22.5 < direction.at<float>(k,m) && direction.at<float>(k,m) <= 67.5)// &&
				// 		 	// magnitudo.at<float>(k,m) > magnitudo.at<float>(k-1,m-1) &&
				// 		 	// magnitudo.at<float>(k,m) > magnitudo.at<float>(k+1,k+1)) 
				// 		{							
				// 			value = true;									
				// 			nonmaxsub.at<float>(k,m) = 255.0;			 
				// 		}
				...
				// 		if( /*nonmaxsub.at<float>(k,m) != 100.0 &&*/ nonmaxsub.at<float>(k,m) == 80.0 && 
				// 			// Tlow < magnitudo.at<float>(k,m) && magnitudo.at<float>(k,m) < Thigh &&
				// 			112.5 < direction.at<float>(k,m) && direction.at<float>(k,m) <= 157.5)// &&
				// 			// magnitudo.at<float>(k,m) > magnitudo.at<float>(k-1,m+1) &&
				// 			// magnitudo.at<float>(k,m) > magnitudo.at<float>(k+1,m-1)) 
				// 		{					
				// 			value = true;										
